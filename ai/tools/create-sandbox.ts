...
import { createSandbox as createSandboxProvider } from "@/lib/executor/provider";
...
export const createSandbox = ({ writer }: Params) =>
  tool({
    ...
    execute: async ({ timeout, ports }, { toolCallId }) => {
      ...
      try {
        // Ports are not currently exposed via E2B; we ignore ports for now.
        const sandboxId = await createSandboxProvider({
        ...
        return (
          ...
          `\nYou can now upload files and run commands.`
        );
      } catch (error) {
